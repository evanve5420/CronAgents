...
- Expose the same actions as `chronagents.ps1` in a visual interface
...
|--------|--------------|---------|
| View agent status / next runs | `GET /api/status` | `chronagents.ps1 status` |
| List agents with schedules | `GET /api/agents` | `chronagents.ps1 list` |
...
After the CLI wrapper (`chronagents.ps1`) has been used enough to validate the command set. The HTTP API should exactly mirror the CLI commands — design the CLI first, then wrap it in HTTP, not the other way around.
