...
| **Default** | `[]` |
| **Description** | Tools to deny. In prompt-only mode (which gets `--allow-all-tools`), use this to restrict specific tools. Values are passed directly to `--deny-tool` and use CLI permission-pattern syntax. |
