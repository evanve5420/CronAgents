...
Launches a lightweight HTTP server on `localhost:9077` serving an interactive HTML dashboard. The dashboard provides live status, agent controls (pause/resume/trigger), run history with details, feedback submission, and question answering — all through a browser UI. Auto-refreshes every 5 seconds.
