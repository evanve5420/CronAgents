...
7. **Execution policies?** — timeout, skip on battery, per-agent quiet hours for scheduled runs (`quietHours`; omit to inherit global, `null` to disable, object to override), retry on process failure, `runIf` (see [RUNIF.md](references/RUNIF.md)), notify on failure (`notifyOnFailure`), notify on success (`notifyOnSuccess`), notification sound (`notificationSound` — e.g. `Alarm3`, `Mail`, `None`), attention level (`raiseAttention` — `all`, `failures-only`, `significant-changes`, or `never`)
