...
7. **Execution policies?** — timeout, skip on battery, retry on failure, `runIf` (see [RUNIF.md](references/RUNIF.md)), notify on failure (`notifyOnFailure`), notify on success (`notifyOnSuccess`), notification sound (`notificationSound` — e.g. `Alarm3`, `Mail`, `None`), attention level (`raiseAttention` — `all`, `failures-only`, `significant-changes`, or `never`)
